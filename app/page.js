...
export default function GrocerySearch() {
  ...
  // Helper for live math in Compare Popup
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '20px auto', padding: '15px', fontFamily: 'sans-serif', color: '#333' }}>
      <h1 style={{ textAlign: 'center', color: '#16a34a' }}>🥘 My Grocery Saver</h1>
      
      {/* Search Bar Row */}
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
        <input type="text" placeholder="Search (chicken, rice...)" style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid #ddd' }} onChange={(e) => setSearchTerm(e.target.value)} />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f8f9fa', borderBottom: '2px solid #eee' }}>
              <th style={thStyle}>Store</th><th style={thStyle}>Item</th><th style={thStyle}>$/lb</th><th style={thStyle}>$/kg</th><th style={{...thStyle, textAlign:'center'}}>Edit</th>
            </tr>
          </thead>
          <tbody>
            {results.map((item, index) => (
              <tr key={item.id} style={{ borderBottom: '1px solid #eee', backgroundColor: index === 0 ? '#dcfce7' : 'white' }}>
                ...
                <td style={tdStyle}>
                  ...
                  <div style={{ fontSize: '11px', color:'#666' }}>{item.brand} ({item.weight_value}{item.weight_unit} @ ${item.price})</div>
                </td>
                <td style={tdStyle}>${parseFloat(item.price_lb || 0).toFixed(2)}</td>
                <td style={tdStyle}>${parseFloat(item.price_kg || 0).toFixed(2)}</td>
                <td style={{ ...tdStyle, textAlign: 'center' }}><button onClick={() => startEdit(item)} style={{ border: 'none', background: 'none', cursor:'pointer' }}>✏️</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* --- POPUP: COMPARE IN-STORE --- */}
      {isCompareOpen && (
        <div style={modalOverlayStyle}>
          <div style={{ ...modalContentStyle, maxWidth: '600px', maxHeight: '95vh', overflowY: 'auto' }}>
            <h3 style={{ marginTop: 0 }}>🔍 Compare In-Store</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px', marginBottom: '15px' }}>
               ...
               <div style={fGroup}><label>Price & Weight</label>
                  <div style={{display:'flex', gap:'4px'}}>
                    <input type="number" placeholder="$" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} style={{...inputStyle, width:'100%'}} />
                    <input type="number" placeholder="Qty" value={formData.weight_value} onChange={e => setFormData({...formData, weight_value: e.target.value})} style={{...inputStyle, width:'100%'}} />
            ...
            {calcCurrentPriceKg() > 0 && (
              <div style={{ padding: '10px', backgroundColor: '#eff6ff', borderRadius: '8px', marginBottom: '15px', border: '1px solid #3b82f6', textAlign:'center' }}>
                <span style={{fontSize:'12px'}}>Your Item: </span><strong>${calcCurrentPriceKg().toFixed(2)}/kg</strong>
              </div>
            )}

            {/* Compact Top 5 Results */}
            <div style={{ borderTop: '1px solid #eee', paddingTop: '10px' }}>
              <div style={{fontSize:'12px', fontWeight:'bold', marginBottom:'5px', color:'#666'}}>Database Matches (Top 5 Cheapest):</div>
              {compareResults.map((item, idx) => (
                <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px', fontSize: '13px', backgroundColor: idx === 0 ? '#dcfce7' : '#fff', borderBottom: '1px solid #f0f0f0' }}>
                  <span>{item.store_name} ({item.brand})</span>
          ...
          <div style={modalContentStyle}>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <select value={formData.weight_unit} onChange={e => setFormData({...formData, weight_unit: e.target.value})} style={inputStyle}><option value="kg">kg</option><option value="lb">lb</option><option value="g">g</option></select>
              </div>
...
const modalContentStyle = { backgroundColor: 'white', padding: '20px', borderRadius: '15px', width: '95%', maxWidth: '400px', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' };
...
const btnSaveStyle = { flex: '1 1 100%', padding: '12px', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' };
